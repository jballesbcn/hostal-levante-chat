...
import React, { useState, useEffect, useRef } from 'export const ChatWidget = ({ knowledge, isEmbedded }) => {
  ...
  // Función de reintento para manejar límites de la API gratuita
  const fetchWithRetry = async (ai, config, text, retries = 2, delay = 2000) => {
    try {
      ...
      // Si es un error 429 (Too Many Requests), reintentamos después de un delay
  ...
  const onSend = async (textOverride) => {
    ...
    try {
      ...
      const config = { 
        ...
        1. PROHIBIDO usar asteriscos (*). Absolutamente prohibido.
        2. Usa viñetas claras con el símbolo "•" para enumerar opciones o servicios.
        3. No uses negrita ni otros formatos de Markdown. Solo texto plano y viñetas "•".
        4. Estructura la respuesta en párrafos cortos. 
        5. Usa saltos de línea simples (un solo ENTER) para separar ideas. No amontones el texto en un solo párrafo.
        6. Sé amable, proactivo y usa emojis de viaje 🏨.
        
        CONTENIDO:
        - Ayuda con info técnica (check-in 15h, check-out 11h, 24h recepción) y consejos locales.
        - Ubicación: Barrio Gótico, Baixada de Sant Miquel 2. Cerca de Liceu (L3).
        - Si no tenemos algo (TV, cocina), ofrece alternativas con empatía.
        - Accesibilidad: El hostal NO está adaptado (escaleras, ascensor pequeño).
...
const UI_TEXT = {
  es: { 
    ...
    error: "El sistema está un poco saturado ahora mismo. ¿Podrías intentar enviarlo de nuevo en unos segundos?",
  ...
  en: { 
    ...
    error: "The system is a bit busy right now. Could you please try again in a few seconds?",
