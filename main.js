...
// Componente para renderizar texto con formato simple (negritas y saltos de línea)
const FormattedMessage = ({ text }) => {
  if (!text) return null;
  
  // Dividimos por negritas **texto**
...
REGLAS DE NEGOCIO:
1. Si preguntan por disponibilidad o precios, redirige siempre a: ${BOOKING_URL}.
2. Sé honesto sobre la accesibilidad (no adaptado, hay escaleras).
3. Recomienda el Barrio Gótico para turismo.
4. No inventes servicios que no tenemos (No hay desayuno, TV ni cocina).`;
      ...
      <div ref=${scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50 hide-scroll">
        ${messages.map((m, idx) => html`
          <div key=${idx} className=${`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className=${`max-w-[90%] p-4 rounded-2xl text-[13px] leading-relaxed shadow-sm ${m.role === 'user' ? 'bg-[#1e3a8a] text-white rounded-tr-none' : 'bg-white text-slate-700 rounded-tl-none border border-slate-100'}`}>
          ...
          ${isAdmin ? html`
            <div className="space-y-4">
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-bold text-slate-700 text-sm italic underline">Base de Conocimientos</h3>
