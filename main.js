...
const askAI = async (history, knowledge, lang) => {
  ...
  if (!apiKey || apiKey === "undefined") return "Error: Configuración de API ausente.";
  try {
    const ai = new GoogleGenAI({ apiKey });
    const context = knowledge.map(k => `${k.title}: ${k.content}`).join('. ');
    const systemInstruction = `Eres el asistente del Hostal Levante Barcelona. Idioma: ${lang}. Reservas: ${BOOKING_URL}. Datos: ${context}`;
  ...
  } catch (error) { return "Lo siento, intenta de nuevo en unos segundos."; }
};
...
const ChatWidget = ({ knowledge, isStandaloneWidget = false }) => {
  ...
  // En modo standalone (dentro de la web), el contenedor debe ocupar todo el espacio pero dejar pasar clicks si no está abierto
  // El botón lo posicionamos abajo a la derecha del iframe
  const containerClass = isStandaloneWidget 
    ? "fixed inset-0 flex flex-col items-end justify-end p-4 pointer-events-none" 
    : "fixed bottom-6 right-6 z-[9999] flex flex-col items-end";

  return html`
    <div className=${containerClass}>
      ${isOpen && html`
        <div className="pointer-events-auto mb-4 w-full sm:w-[350px] h-[500px] max-h-[80vh] bg-white rounded-[2rem] shadow-2xl border border-slate-200 overflow-hidden flex flex-col animate-chat">
          <div className="bg-[#1e3a8a] p-4 text-white flex justify-between items-center shrink-0">
            ...
            <button onClick=${() => setIsOpen(false)} className="w-8 h-8 rounded-full hover:bg-white/10"><i className="fas fa-times"></i></button>
          </div>
          ...
          <div className="p-3 bg-white border-t border-slate-100 flex space-x-2 shrink-0 pointer-events-auto">
            <input value=${input} onChange=${e => setInput(e.target.value)} onKeyDown=${e => e.key === 'Enter' && onSend()} placeholder="..." className="flex-1 bg-slate-100 rounded-xl px-4 py-2 text-sm outline-none" />
            <button onClick=${onSend} className="bg-[#1e3a8a] text-white w-10 h-10 rounded-xl flex items-center justify-center"><i className="fas fa-paper-plane text-xs"></i></button>
          </div>
        </div>
      `}
      <button 
        onClick=${() => setIsOpen(!isOpen)} 
        className="pointer-events-auto w-14 h-14 bg-[#1e3a8a] text-white rounded-full shadow-2xl pulse-blue border-4 border-white flex items-center justify-center cursor-pointer transition-transform active:scale-90"
      >
...
const App = () => {
  ...
  // AHORA DETECTAMOS AMBOS: widget o embed
  const isWidgetMode = params.has('widget') || params.has('embed');

  // SI ES MODO WIDGET/EMBED (PARA LA WEB), NO RENDERIZAMOS EL PANEL DE ADMIN
  if (isWidgetMode) {
    return html`<${ChatWidget} knowledge=${knowledge} isStandaloneWidget=${true} />`;
  }

  // SI NO, RENDERIZAMOS EL PANEL DE ADMIN (PARA GESTIÓN)
  return html`
    <div className="min-h-screen bg-slate-100 p-4 md:p-12">
      <div className="max-w-4xl mx-auto bg-white rounded-[2.5rem] shadow-xl overflow-hidden border">
        <div className="p-6 bg-[#1e3a8a] text-white flex justify-between items-center">
          <div className="flex items-center space-x-3">
             ...
             <h1 className="font-bold uppercase tracking-widest text-sm">Levante AI Admin</h1>
          </div>
          <button onClick=${() => setKnowledge([{id:Date.now(), title:'', content:''}, ...knowledge])} className="bg-white/20 px-4 py-2 rounded-lg text-[10px] font-bold hover:bg-white/30 transition-colors">AÑADIR DATO</button>
        </div>
        <div className="p-8 space-y-4">
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-4">Información para la IA:</p>
          ${knowledge.map(k => html`
            <div key=${k.id} className="flex gap-2 p-3 bg-slate-50 rounded-xl border border-slate-100">
              <input className="bg-white border p-2 rounded-lg text-xs w-1/4" placeholder="Título" value=${k.title} onChange=${e => setKnowledge(knowledge.map(x => x.id === k.id ? {...x, title: e.target.value} : x))} />
              <textarea className="bg-white border p-2 rounded-lg text-xs flex-1" placeholder="Datos..." value=${k.content} onChange=${e => setKnowledge(knowledge.map(x => x.id === k.id ? {...x, content: e.target.value} : x))} />
      ...
      <!-- En el modo admin también mostramos el widget para probarlo -->
